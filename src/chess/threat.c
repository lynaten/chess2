...
int threats_mg(ChessPosition *pos1, ChessPosition *pos2, bool colorflipped)
{
    ChessPosition *pos = colorflipped ? pos2 : pos1;
    int v = 0;
    v += 69 * hanging(pos, NULL, NULL);
    v += king_threat(pos, NULL, NULL) > 0 ? 24 : 0;
    v += 48 * pawn_push_threat(pos, NULL, NULL);
    v += 173 * threat_safe_pawn(pos, NULL, NULL);
    v += 60 * slider_on_queen(pos, NULL, NULL);
    v += 16 * knight_on_queen(pos, NULL, NULL);
    v += 7 * restricted(pos, NULL, NULL);
    v += 14 * weak_queen_protection(pos, NULL, NULL);
    for (int x = 0; x < 8; x++)
    {
        for (int y = 0; y < 8; y++)
        {
